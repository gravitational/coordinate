...
// LeaderConfig sets leader election configuration options
...
// NewLeader returns a new instance of leader election client
func NewLeader(cfg LeaderConfig) (*Client, error) {
	if cfg.Clock == nil {
...
func (l *Client) AddWatchCallback(key string, retry time.Duration, fn leader.CallbackFn) {
	go func() {
...
func (l *Client) AddWatch(key string, retry time.Duration, valuesC chan string) {
	...
	go func() {
		...
		backoff := &leader.FreebieExponentialBackOff{
